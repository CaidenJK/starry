...
Starry outputs to `[STARRY_DIR]/bin/[SYSTEM_NAME]`. Starry's static library builds to `/lib` and the included glfw subproject builds to a shared library under `/dyn`.

Starry's renderer currently supports window resizing, MSAA, ImGui GUI integration, and is build on the Vulkan Graphics API and GLFW.
...
![Example Render](/docs/media/example1.png)